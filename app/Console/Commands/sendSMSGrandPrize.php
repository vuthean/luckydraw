...
class sendSMSGrandPrize extends Command
{
    use LuckyDrawDate;
     ...
     * @var string
     */
    protected $signature = 'sms:send-grand-prize';
     ...
     * @var string
     */
    protected $description = 'Send all customer ticket for grand prize.';
    ...
    public function handle()
    {
        ...
        Log::info("Schedule run to send SMS on date: {$currentDate} is working fine");

        $luckyDrawDate = $this->getLuckyDrawDate();
        $month         = $luckyDrawDate->month;

        /** get all ticket for current month */
        $tickets = Ticket::whereNull('spind_grand_prize_at')->whereNull('win_at')->get();
        if (collect($tickets)->isEmpty()) {
            Log::info("Cannot send SMS since we cannot find any customer tickets on month :{$month}");
            return false;
        }
