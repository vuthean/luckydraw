            ...
            success: function(data) {
                ...
                for (const key in data[0]) {
                    if (Object.hasOwnProperty.call(data[0], key)) {
                        ...
                        $('#1_win_name').html(element.customer_name);
                        $('#1_win_cif').html(element.ticket_number);
                    }
                }

                for (const key in data[1]) {
                    if (Object.hasOwnProperty.call(data[1], key)) {
                        const element = data[1][key];
                        $('#2_win_name').html(element.customer_name);
                        $('#2_win_cif').html(element.ticket_number);
                    }
                }

                for (const key in data[2]) {
                    if (Object.hasOwnProperty.call(data[2], key)) {
                        const element = data[2][key];
                        var txtNum = parseInt(key) + 1;
                        $('#3_win_name_' + txtNum).html(element.customer_name);
                        $('#3_win_cif_' + txtNum).html(element.ticket_number);
                    }
                }
            },
...
<script type="text/javascript">
    ...
    $('#btn-example10-start').click(function() {
        ...
        if ($('#prize_id').val() == '3') {
            if ($('#1_win_name').html() !== '') {
                alert('Winner For This Prize Have Been Found! Please Go To The Next Prize.');
                return;
            }
        } else if ($('#prize_id').val() == '2') {
            if ($('#2_win_name').html() !== '') {
                alert('Winner For This Prize Have Been Found! Please Go To The Next Prize.');
                return;
            }
        } else if ($('#prize_id').val() == '1') {
            if ($('#3_win_name_10').html() !== '') {
                alert('All Winners For This Prize Have Been Found! Please Go To The Next Prize.');
                return;
            }
        }
        ...
        if ($(this).html() == 'Start') {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var customer_CIF = response['customer_CIF'];
                    ...
                    var num6 = parseInt(ticket_number.slice(5, 6)) + 1;
                    var num7 = parseInt(ticket_number.slice(6, 7)) + 1;
                    var num8 = parseInt(ticket_number.slice(7, 8)) + 1;
                    var num9 = parseInt(ticket_number.slice(8, 9)) + 1;
            ...
            $(this).html("Start");
            $(this).css('background', 'linear-gradient(90deg, rgba(29,193,152,1) 0%, rgba(107,251,206,1) 100%)');
    ...
    timer = setInterval(function() {
        ...
        if (numKeeptrack < -15) {
            ...
            // $('#btn-example10-stop').css({
            //     "background-image": "url('{{asset('PUBLIC/src/image/stop_btn_disable.png')}}')"
            // });
            $('#btn-example10-start').attr("disabled", false);
            // $('#btn-example10-start').css({
            //     "background-image": "url('{{asset('PUBLIC/src/image/start_btn.png')}}')"
            // });
            $('#confetti').show();
